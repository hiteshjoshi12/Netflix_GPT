...
const Gptsearchbar = () => {
  ...
  //     ". only give me names of 8 movies, comma seperated like the example given ahead. Example result: Gadar, Sholay, Don, Golmal, ddlj";

  //   const result = await chatSession.sendMessage(query);
  //    const data = JSON.parse(result.response.text());
  //   const movieString = data.searchText.current.value;
  //   const movieList = movieString.split(",").map((movie) => movie.trim());
  //   console.log(data);
  //   // search each movie in tmdb api

  //   const PromiseMovieDataArray = movieList.map(movie=> searchMovieTmdb(movie));

  //   const tbdbResults =await Promise.all(PromiseMovieDataArray);
  //   console.log(tbdbResults);
